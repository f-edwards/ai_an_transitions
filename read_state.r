...
pop<-pop %>% 
  ...
  ungroup() %>% 
  rename(staterr = state, state = st_fips) %>% 
  mutate(state = as.numeric(state))

afcars<-read_csv("~/Projects/cps_lifetables/data/afcars_imputed_all_cases_with_pop.csv",
...
ncands_first_nat<-ncands_first %>%
  group_by(.imp, staterr, chage, race_ethn, year) %>%
  summarise(var = n()) %>%
  rename(age = chage) %>%
  left_join(pop) %>%
  ungroup() %>%
  write_csv("./data/ncands_first_inv_st.csv")
...
ncands_first_subst_nat<-ncands_first_subst %>%
  ...
  left_join(pop) %>%
  ungroup() %>%
  write_csv("./data/ncands_first_sub_st.csv")
